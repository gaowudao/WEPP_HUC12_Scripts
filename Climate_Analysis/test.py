...
def get_cli_dfs(mod, period, years):
    '''
    ...
    runs_dir = str(HUC12_path + 'Runs/test/' + mod + '_{}/'.format(period))
    ...
    for file in os.listdir(runs_dir):
        ...
        if file.endswith('.cli'):
            ...
            def group_by_season(months):
                ...
                #get total seasonal precip by year
                precip = df.groupby('year')['prcp'].sum()
                #No. days with prcp above 10mm for specificed months in every year
                R10 = df_G1[df_G1['prcp'].astype(float) > 10].groupby('year')['prcp'].count()
                #Same as R10 but for prcp > 25mm
                R25 = df_G1[df_G1['prcp'].astype(float) > 25].groupby('year')['prcp'].count()
                ...
                season = pd.DataFrame({'precip':precip,'R10':R10, 'R25':R25,\
                                        'R95pT':R95pT}, index = year_index)
                return season
...
r1_19 = list(get_cli_dfs(mod_rcps[0], '19', 55))
r2_19 = list(get_cli_dfs(mod_rcps[1], '19', 55))
r1_59 = list(get_cli_dfs(mod_rcps[0], '59', 40))
r2_59 = list(get_cli_dfs(mod_rcps[1], '59', 40))
r1_99 = list(get_cli_dfs(mod_rcps[0], '99', 40))
r2_99 = list(get_cli_dfs(mod_rcps[1], '99', 40))
...
var_lst = ['precip', 'R10', 'R25']

#create output dictionary for each model
trends_dic = {}
...
def analyze_trends(r19, r59, r99):
    ...
    trends_df = pd.DataFrame({'fall_pr':fall_trends[0:3], 'fall_R10':fall_trends[3:6],\
                              'fall_R25':fall_trends[6:9], 'spring_pr':spring_trends[0:3],\
                              'spring_R10':spring_trends[3:6], 'spring_R25':spring_trends[6:9],\
                              'summer_pr':summer_trends[0:3], 'summer_R10':summer_trends[3:6],\
                              'summer_R25':summer_trends[6:9], 'winter_pr':winter_trends[0:3],\
                              'winter_R10':winter_trends[3:6], 'winter_R25':winter_trends[6:9]})

    return(trends_df)


#Assign trends_df output to the trends_dic dictionary
trends_dic[mod_rcps[0]] = analyze_trends(r1_19, r1_59, r1_99)
trends_dic[mod_rcps[1]] = analyze_trends(r2_19, r2_59, r2_99)
