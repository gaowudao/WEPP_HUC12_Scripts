...
def analyze_RO_TSS(add_years, start_crop1_yrs, start_crop2_yrs, crop1_obs_yrs, crop2_obs_yrs, \
    ...
    def Get_Monthly_WEPP_Data(mod_lab, crop_yrs, crop_name):
        ...
        for hill in out_hill_names:
            ...
            HS_month_events_RO = ind_crop.groupby('Month')['RO'].count() / len(crop_yrs)
            HS_month_RO = ind_crop.groupby('Month')['RO'].sum() / len(crop_yrs)

            #Repeat for TSS and convert to tons/ha
            HS_month_events_TSS = ind_crop.groupby('Month')['Mx-det'].count() / len(crop_yrs)
            HS_month_TSS = (ind_crop.groupby('Month')['Mx-det'].sum() * 11.0231) / len(crop_yrs)

            #create dataframe
            HS_month_comb = pd.DataFrame({'RO':HS_month_RO, 'Total RO Events':HS_month_events_RO,\
                                          'Soil_detach':HS_month_TSS, 'Avg # Erosion Events':HS_month_events_TSS})
    ...
    for mod_lab in mod_labels:
        ...
        WEPP_dic[str(wshed + '_' + crop1_name + '_' + mod_lab)] = WEPP_crop1_data.fillna(0)
        ...
        WEPP_dic[str(wshed + '_' + crop2_name + '_' + mod_lab)] = WEPP_crop2_data.fillna(0)
    ...
    def sep_obs_crops(crop_obs_yrs):
        ...
        crop_out = pd.DataFrame({'RO':crop_RO, 'Avg # RO Events':RO_events,\
                                 'Soil_detach':crop_TSS, 'Avg # Erosion Events':SE_events},index = months).fillna(0)
        return crop_out
    ...
    obs_dic[str(wshed + '_' + crop1_name)] = crop1_obs
    obs_dic[str(wshed + '_' + crop2_name)] = crop2_obs
